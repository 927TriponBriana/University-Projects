...
from DirectedGraph import TripleDictGraph, write_graph_to_file, read_graph_from_file
...
class UI:
    ...
    def topological_sort_ui(self):
        sorted_list = self._graphs[self._current].base_topological_sort()
        if len(sorted_list) == 0:
            ...
            if start_vertex not in self._graphs[self._current].vertices or end_vertex not in self._graphs[self._current].vertices:
                raise ValueError("One vertex/both vertices entered might not exist!")
            else:
                distance, prev = self._graphs[self._current].highest_cost_path(start_vertex, end_vertex)
                if distance == float("-inf"):
                    raise ValueError("There is no walk from {} to {}".format(start_vertex,end_vertex))
                path = []
                vertex = end_vertex
                path.append(vertex)
                while prev[vertex] != -1:
    ...
    def print_menu(self):
        print("MENU:\n"
              ...
              "24. Check if the graph is a DAG and sort the activities topologically; if the graph is a DAG find the highest cost path between 2 vertices.")
    ...
    def start(self):
        ...
        command_dict = {"1": self.create_random_graph_ui, "2": self.read_graph_from_file_ui,
                        ...
                        "23": self.minimum_cost_path_ui, "24": self.topological_sort_ui}
        while not done:
            try:
                self.print_menu()
