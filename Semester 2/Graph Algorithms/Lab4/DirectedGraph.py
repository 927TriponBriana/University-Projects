...
class TripleDictGraph:
    ...
    def topological_sort_dfs(self, vertex, sorted_list, fully_processed, in_process):
        """
        ...
        for other_vertex in self._dictionary_in[vertex]: #process all the inbound verices of the current vertex
            if other_vertex in in_process:
                return False
            elif other_vertex not in fully_processed: #if one of the inbound vertices of the current one is not in processed, process it and its inbound vertices
                ok = self.topological_sort_dfs(other_vertex, sorted_list,fully_processed,in_process)
    ...
    def base_topological_sort(self):
        ...
        in_process = set() #set for te vertices that are in process
        for vertex in self._vertices: #go through all the vertices
    ...
    def highest_cost_path(self, start_vertex, end_vertex):
        """FUnction to compute the highest cost path from the start vertex to the end one"""
        topological_order_list = self.base_topological_sort()
        dist = {}
        prev = {}
        inf = float('-inf')
        for vertex in topological_order_list:
            dist[vertex] = inf
            prev[vertex] = -1
        dist[start_vertex] = 0
