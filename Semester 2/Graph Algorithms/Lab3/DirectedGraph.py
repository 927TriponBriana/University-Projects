...
class TripleDictGraph:
    ...
    def minimum_cost_path(self, vertex_begin, vertex_end):
        ...
        """
        dist = {}  # dictionary that associates to each accessible vertex the cost of the minimum cost walk from begin to end
        ...
        while changed:
            ...
            for (x, y) in self._dictionary_cost.keys():  # for every edge, check if the distance is minimum
                # if it is, add it to the path and update the distance to the vertex y from the starting vertex
