...
class UndirectedGraph:
    ...
    def DFSUtil(self, v, visited, traversal):
        ...
        for neighbour in self._dictionary_bound[v]:
            if neighbour not in visited:
                self.DFSUtil(neighbour, visited, traversal)  # go recursively through all the bound vertices that are not visited yet
    ...
    def primsMST(self):
        ...
        for x in self.parse_bound(s):
            heapq.heappush(q, (self.find_cost(s, x), s, x)) # push the vertices and the costs onto the heap
        treeVertices = {s}
        treeEdges = []  # edges of the tree
        treeCost = 0
        while len(q) > 0:  # algorithm stops when there are no edges left in the heap
            cost, x, y = heapq.heappop(q) # pop and return the smallest item from the heap
            ...
            for z in self.parse_bound(y):  # go through the bound edges and find other candidate edge
                if z not in treeVertices:
                    heapq.heappush(q, (self.find_cost(y, z), y, z)) # push the edge between y an z and its cost onto the heap
