...
class Settings(object):
    ...
    def readFile(self):
        try:
            with open('..\data\settings.properties') as file:
                for line in file:
                    list = line.split(" ")
                    if list[0] == "repository":
                        mode_repository = str(list[2].strip('\n'))
                    elif list[0] == "students":
                        students = list[2].strip('\n').strip("\"")
                    elif list[0] == "disciplines":
                        disciplines = list[2].strip('\n').strip("\"")
                    elif list[0] == "grades":
                        grades = list[2].strip('\n').strip("\"")
                    elif list[0] == "ui":
                        interface_mode = list[2].strip('\n').strip("\"")
        except IOError:
            print("Fille missing")
